...
detune_mod_sel=0, detune_mod_freq=0.5, detune_mod_amt=0.0, ratio1_mod_sel=0, ratio1_mod_freq=0.5, ratio1_mod_amt=0.0, fm2_mod_sel=0, fm2_mod_freq=0.5, fm2_mod_amt=0.0, 
fbx_mod_sel=0, fbx_mod_freq=0.5, fbx_mod_amt=0.0, ratio2_mod_sel=0, ratio2_mod_freq=0.5, ratio2_mod_amt=0.0;
var sound, fm1_mod, detune_mod, ratio1_mod, fm2_mod, fbx_mod, ratio2_mod, xxx, yyy, zzz, uuu, vvv, www;
...
fm2_mod = Select.ar(fm2_mod_sel, [SinOsc.ar(fm2_mod_freq, 0, 0.5, 0.5), Saw.ar(fm2_mod_freq, 0.5, 0.5), LFDNoise1.ar(fm2_mod_freq, 0.5, 0.5)]);
fbx_mod = Select.ar(fbx_mod_sel, [SinOsc.ar(fbx_mod_freq, 0, 0.5, 0.5), Saw.ar(fbx_mod_freq, 0.5, 0.5), LFDNoise1.ar(fbx_mod_freq, 0.5, 0.5)]);
ratio2_mod = Select.ar(ratio2_mod_sel, [LFDNoise0.ar(ratio2_mod_freq, 0.5, 0.5), LFTri.ar(ratio2_mod_freq, 0.0, 0.5, 0.5).abs, LFDNoise1.ar(ratio2_mod_freq, 0.5, 0.5)]);
...
uuu = Clip.kr(ratio2 + (ratio2_mod * ratio2_mod_amt), 0.0, 1.0);
vvv = Clip.kr(fbx + (fbx_mod * fbx_mod_amt), 0.0, 1.0);
www = Clip.kr(fm2 + (fm2_mod * fm2_mod_amt), 0.0, 1.0);
sound = MiOmi.ar(SoundIn.ar(0), gate, pit, contour, yyy, level1, level2, xxx, uuu, zzz, www, fb, vvv, filter_mode, cutoff, reson, strength, env, rotate, space, mul, add);
sound = MiVerb.ar(sound, revtime, drywet, damping, hp, freeze, diffusion);
SendReply.kr(Impulse.kr(15),"/tr", [xxx, yyy, zzz, uuu, vvv, www]);
...
responder = OSCFunc({arg msg; freq = msg[3]; freq2 = msg[4]; freq3 = msg[5]; freq4 = msg[6]; freq5 = msg[7]; freq6 = msg[8]; }, '/tr', context.server.addr);
...
this.addPoll("sendlfo4", {var val = freq4; val});
  
this.addPoll("sendlfo5", {var val = freq5; val});
  
this.addPoll("sendlfo6", {var val = freq6; val});
...
this.addCommand("ratio2_mod_sel", "f", {|msg| synth.set(\ratio2_mod_sel, msg[1]-1);});
   
this.addCommand("ratio2_mod_freq", "f", {|msg| synth.set(\ratio2_mod_freq, msg[1]);});
   
this.addCommand("ratio2_mod_amt", "f", {|msg| synth.set(\ratio2_mod_amt, msg[1]);});
...
this.addCommand("fm2_mod_sel", "f", {|msg| synth.set(\fm2_mod_sel, msg[1]-1);});
   
this.addCommand("fm2_mod_freq", "f", {|msg| synth.set(\fm2_mod_freq, msg[1]);});
    
this.addCommand("fm2_mod_amt", "f", {|msg| synth.set(\fm2_mod_amt, msg[1]);});
    
this.addCommand("fb", "f", {|msg| synth.set(\fb, msg[1]);});
    
this.addCommand("fbx", "f", {|msg| synth.set(\fbx, msg[1]); });
this.addCommand("fbx_mod_sel", "f", {|msg| synth.set(\fbx_mod_sel, msg[1]-1);});
   
this.addCommand("fbx_mod_freq", "f", {|msg| synth.set(\fbx_mod_freq, msg[1]);});
    
this.addCommand("fbx_mod_amt", "f", {|msg| synth.set(\fbx_mod_amt, msg[1]);});
